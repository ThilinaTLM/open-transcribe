    ...
    fn default() -> Self {
        ...
        let no_speech_threshold = std::env::var("WHISPER_NO_SPEECH_THRESHOLD")
            .map(|v| {
                ...
                debug!("WHISPER_NO_SPEECH_THRESHOLD={}, parsed as: {}", v, threshold);
                threshold
            })
            .unwrap_or_else(|_| {
        ...
        let num_threads = std::env::var("WHISPER_NUM_THREADS")
            ...
            .unwrap_or_else(|_| {
                debug!("WHISPER_NUM_THREADS not set, defaulting to: {} (available parallelism)", default_threads);
                default_threads
            });
        ...
        if config.audio_context < 1 || config.audio_context > 4096 {
            warn!("Audio context {} is outside recommended range (1-4096)", config.audio_context);
        }
        
        if config.no_speech_threshold < 0.0 || config.no_speech_threshold > 1.0 {
            warn!("No speech threshold {} is outside valid range (0.0-1.0)", config.no_speech_threshold);
        }
        
        if config.num_threads < 1 {
            warn!("Number of threads {} is invalid, should be >= 1", config.num_threads);
        }

        info!("WhisperConfig created: model_path={:?}, use_gpu={}, language={}, audio_context={}, no_speech_threshold={}, num_threads={}", 
              config.model_path, config.use_gpu, config.language, 
              config.audio_context, config.no_speech_threshold, config.num_threads);
