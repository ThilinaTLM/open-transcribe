...
impl SimpleTranscriber {
    pub fn new(config: WhisperConfig) -> Result<Self> {
        info!("Creating new SimpleTranscriber with model: {:?}", config.model_path);
        ...
        let ctx = WhisperContext::new_with_params(
            ...
            error!("Failed to load Whisper model from {:?}: {}", config.model_path, e);
            anyhow::anyhow!("Failed to load model: {}", e)
        })?;

        info!("Whisper model loaded successfully from {:?}", config.model_path);
    ...
    pub fn transcribe(&self, audio_data: &InputAudio) -> Result<TranscribeOutput> {
        ...
        debug!("Starting transcription: {} samples, {}Hz, {} channels", 
               audio_data.data.len(), audio_data.sample_rate, audio_data.channels);

        // Resample audio to 16kHz if needed
        let resampled_audio = if audio_data.sample_rate != 16000 {
            info!("Resampling audio from {}Hz to 16kHz", audio_data.sample_rate);
        ...
        if resampled_audio.len() < 16000 {
            warn!("Audio is too short: {} samples (less than 1 second)", resampled_audio.len());
        ...
        debug!("Transcription parameters: language={}, audio_ctx={}, no_speech_threshold={}, threads={}", 
               self.config.language, self.config.audio_context, 
               self.config.no_speech_threshold, self.config.num_threads);
        ...
        debug!("Running whisper transcription on {} samples", mono_audio.len());
        ...
        debug!("Extracting {} segments from transcription result", num_segments);
        ...
        for i in 0..num_segments {
            ...
            debug!("Segment {}: {}ms-{}ms, confidence: {:.3}, text: {:?}", 
                   i, start, end, confidence, text.trim());
        ...
        info!("Transcription complete: {} segments, {} characters, {:.1}s audio processed in {:?} (RTF: {:.2}x)", 
              segments.len(), combined.len(), audio_duration_seconds, total_duration, real_time_factor);
    ...
    fn calculate_segment_confidence(&self, state: &whisper_rs::WhisperState, segment_idx: i32) -> Result<f32> {
        ...
        if n_tokens == 0 {
            debug!("Segment {} has no tokens, returning confidence 0.0", segment_idx);
            return Ok(0.0);
        }
        ...
        debug!("Segment {} confidence calculation: {} tokens, avg_logprob: {:.4}, confidence: {:.4}", 
